...
| Kotlin | 2.4.20 | 2026-09-07 | [Kotlin releases](https://kotlinlang.org/docs/releases.html#release), [Kotlin 2.4.20](https://github.com/JetBrains/kotlin/releases/tag/v2.4.20) | Current stable tooling release in the Kotlin 2.4 line. The Compose compiler is supplied by the Kotlin plugin/toolchain, so one coordinated Kotlin version is retained. AGP 9.4.1 embeds Kotlin 2.2.10, so this project explicitly uses AGP's documented Kotlin opt-out until an AGP release embeds a compatible current compiler. |
| `compileSdk` | 37 | 2026-02-16 | [Android 17/API 37](https://developer.android.com/about/versions/17), [Compose compiler setup](https://developer.android.com/develop/ui/compose/setup-compose-dependencies-and-compiler) | Compile against API 37 because the selected stable Compose BOM includes Compose libraries that require `compileSdk 37`; compilation does not opt the app into Android 17 behavior. `targetSdk` remains stable API 36. |
...
### Kotlin compiler compatibility decision

AGP 9.4.1's published module metadata embeds Kotlin Gradle Plugin and standard-library version 2.2.10. The selected Compose BOM and current Compose compiler setup require the coordinated Kotlin 2.4.20 toolchain. The project therefore uses the temporary AGP 9.x compatibility flags `android.builtInKotlin=false` and `android.newDsl=false`, then applies Kotlin Android and Compose compiler 2.4.20 explicitly. This is a documented compatibility constraint, not an obsolete-tool choice. The flags must be removed in the first AGP release whose built-in Kotlin version is verified compatible with Kotlin 2.4.20 and the selected Compose BOM.
